...
│                                                                 │
│  // Right Identity                                              │
│  Optional.of(5).flatMap(Optional::of)                          │
│      == Optional.of(5)   ✅                                    │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  성공 트랙 (Result.Success / Validation.Valid):                │
│  ════════════════════════════════════════════════════════▶     │
│           │              │              │                       │
│           ▼              ▼              ▼                       │
│       [검증1]        [검증2]        [검증3]                    │
...
│           ▼              ▼              ▼                       │
│  ────────────────────────────────────────────────────────▶     │
│  실패 트랙 (Result.Failure / Validation.Invalid):              │
│                                                                 │
│  Result: 하나라도 실패 → 실패 트랙으로 직행                    │
│  Validation: 모든 실패 수집 → 마지막에 합쳐서 반환             │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
│  2. 성능 최적화                                                 │
│     필요한 것만 계산 (쇼트서킷)                                 │
│     stream.filter(...).findFirst()는 첫 매치에서 멈춤          │
│                                                                 │
│  3. 메모리 효율                                                 │
│     거대한 파일도 한 줄씩 처리 가능                             │
│                                                                 │
│  4. 조합 용이                                                   │
│     중간 컬렉션 없이 복잡한 파이프라인 구성                     │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
